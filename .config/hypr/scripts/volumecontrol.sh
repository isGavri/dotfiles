...
notify_vol ()
{
    ...
    notify-send  -a "Volume Control" -r 91190 -t 800 -i "${ico}" "${vol}${bar}" "${nsink}"
}
...
notify_mute ()
{
    ...
    if [ "${mute}" == "true" ] ; then
        notify-send -a "Volume Control" -r 91190 -t 800 -i "${icodir}/muted-${dvce}.svg" "muted" "${nsink}"
    else
        notify-send -a "Volume Control" -r 91190 -t 800 -i "${icodir}/unmuted-${dvce}.svg" "unmuted" "${nsink}"
    fi
}
